...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        torch.random.manual_seed(index)  # make randomized image deterministic by index
        img = torch.rand(*self.image_dim)*255
        classification = torch.randint(0, self.num_classes, (1,))
        torch.manual_seed(2741992)  # return index to previous state
        return img, classification.item()
